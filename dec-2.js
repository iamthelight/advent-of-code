...
3-5 x: xxqxm`;
var arr = passList.split('\n');

var getIndex = (str, indexChar) => {
    return str.indexOf(indexChar) + 1;
}

var createObject = (str) => {
    var newObj = {
        'min' : str.substring(0, str.indexOf('-')),
        'max' : str.substring(getIndex(str, '-'), str.indexOf(' ')),
        'character': str.substring(getIndex(str, ' '), str.indexOf(':')),
        'password': str.substring(getIndex(str, ':')).trim(),
    }
    console.log(newObj);
    return newObj; 
}
...
var validate = (data) => {
    let regex = new RegExp(data.character, 'g');
    let characterCount = (data.password.match(regex) || []).length;
    console.log(regex);
    console.log(characterCount);
    console.log(`${characterCount} >= ${data.min} : ${characterCount >= data.min}`);
    console.log(`${characterCount} <= ${data.max} : ${characterCount <= data.max}`);
    if(characterCount >= data.min && characterCount <= data.max){
        console.log(true);
        return true;
    }
    console.log(false);
...
var validateTobbogan = (data) => {
    let expression = `${data.character}`;
    let regex = new RegExp(expression, 'g');


    if((data.password.charAt(data.min-1) === data.character && data.password.charAt(data.max-1) !== data.character) || (data.password.charAt(data.min-1) !== data.character && data.password.charAt(data.max-1) === data.character))
    {
        console.log(true);
        return true;
    }
...
createObject(arr[7]);
console.log(validateTobbogan(createObject(arr[7])));

let ctr = 0;

for (i = 0; i < arr.length; i ++) {
    if(validateTobbogan(createObject(arr[i]))){
        ctr++;
    }
}
