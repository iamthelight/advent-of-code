...
var getKeyValuePairs = (str) => {
    var a= str.replace(/\n/g, ' ').split(' ');
    var keyValuePairs = new Object();
 
    for (var i = 0; i< a.length; i++) {
        keyValuePairs[a[i].substring(0, a[i].indexOf(':'))] = a[i].substring(a[i].indexOf(':')+1);
    }
...
let validateField = (key, value) => {
    switch (key) {
        ...
        case 'eyr':
            ...
            break;
        case 'hgt':
            if(value.search('in') > 0){
                let height = parseInt(value.substring(0,value.indexOf('i')));
                if(height >= 59 && height <= 76 ){
                    return true;
                }
                return false;
            } else if(value.search('cm') > 0){
                let height = parseInt(value.substring(0,value.indexOf('c')));
        ...
        case 'hcl':
            if (value.length === 7){
                const regex = RegExp(/[#](\d|[a-f]){6}/gmi);
                if(regex.test(value)){
                    return true;
                }
            }
        ...
        case 'ecl':
            if (value.length === 3){
                const regex = RegExp(/(amb|blu|brn|gry|grn|hzl|oth)/gmi);
                if(regex.test(value)){
                    return true;
                }
            }
...
let revalidate = (passport, keys) => {
    let revalidatedValid = 0;
    let validFields = 0;
    keys.forEach((key, index) => {
        if(validateField(key, passport[key])){
            // console.log(`${key}:${passport[key]} is VALID`)
            validFields++;
        }
        // else {
            // console.log(`${key}:${passport[key]} is INVALID`)
        // }
    });
        // console.log(`validFields/keys: ${validFields}/${keys.length}`);
        // console.log(`______________________`);
    if(validFields === keys.length){
        return true ;
    }
    return false;
}
...
let validCount = 0;
let initialValidationCount = 0;
for( i = 0; i < arr.length; i++) {
    let passport = getKeyValuePairs(arr[i]);
    let keys = getKeys(passport);

    if(keys.length === 7) {
        if(!keys.find(key => key === 'cid')) {   
            initialValidationCount++;         
            if(revalidate(passport, keys)){
                validCount++;
            }
        }
    } 
...
console.log(`PASSPORTS: ${arr.length}`);
console.log(`INITIAL VALID: ${initialValidationCount}`);
console.log(`FINAL VALID: ${validCount}`);
